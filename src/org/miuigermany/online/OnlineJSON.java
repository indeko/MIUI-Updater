package org.miuigermany.online;
...
public class OnlineJSON {
	...
	public OnlineJSON(){
		...
		this.miVersion = new Update("MIUI", "", "", version);
		buscarVersion();
	}
	...
	public ArrayList<Update> buscarVersion(){
		...
		try {
			...
			JSONArray incrementals = this.json.getJSONArray("Incremental");
			for(int i = 0; i < incrementals.length(); i++){
				String versionStr = incrementals.getJSONObject(i).getString("version");
				Log.d("MIUIUPDATER",versionStr);
				String ArrayList[] = versionStr.split("\\.");
				int version[] = new int[ArrayList.length];
				for(int j = 0; j < ArrayList.length; j++){
					version[j] = Integer.parseInt(ArrayList[j]);
				}
				Update versionAux = new Update(incrementals.getJSONObject(i).getString("name"), incrementals.getJSONObject(i).getString("url"), incrementals.getJSONObject(i).getString("descUrl"), version);
				...
				if(Arrays.equals(versionIndicada,miVersion.getVersion())){
					Log.d("MIUIUPDATER","Something must be said");
					if(miVersion.compareTo(versionAux) == -1){
						Log.d("MIUIUPDATER","Y aquí aún más");
					}
						incrementales.add(versionAux);
				}
			}
			versions = this.json.getJSONArray("UpdateList");
			for(int i = 0; i < versions.length(); i++){
				...
				Update versionAux = new Update(versions.getJSONObject(i).getString("name"), versions.getJSONObject(i).getString("url"), versions.getJSONObject(i).getString("descUrl"), version);
				if(miVersion.compareTo(versionAux) == -1){
					versiones.add(versionAux);
				}
			}
			
			JSONArray patchs = this.json.getJSONArray("patches");
			for(int i = 0; i < patchs.length(); i++){
				...
				Parche parcheAux = new Parche(patchs.getJSONObject(i).getString("name"), patchs.getJSONObject(i).getString("url"),patchs.getJSONObject(i).getString("descUrl"),patchMaxInt,patchMinInt);
	...
	public String getModVersion(String prop) {
        ...
        try {
            ...
            Log.e("MIUIUPDATER", "Unable to read sysprop "+prop, ex);
            return null;
        }
        finally {
            if (input != null) {
                try {
                    ...
                    Log.e("MIUIUPDATER", "Exception while closing InputStream", e);
                }
            }
        }
        return line;
